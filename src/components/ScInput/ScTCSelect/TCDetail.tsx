...
const TCDetail = ({ visible, currentItem, backAction, t, theme }: ScDetailProps<ScDataTypeList.TC>) => {
    return (
        <Portal>
                ...
                <Dialog.ScrollArea style={{ maxHeight: "75%" }} >
                    {currentItem === undefined || currentItem.id === 0
                        ...
                        : <ScrollView style={{ maxHeight: "100%" }}>
                            <Text variant="bodyMedium" maxFontSizeMultiplier={1.4} selectable>{t("tcName")} : {currentItem.name}</Text>
                            <Text variant="bodyMedium" maxFontSizeMultiplier={1.4} selectable>{t("tcClassHour")} : {currentItem.classHour}</Text>
                            <Text variant="bodyMedium" maxFontSizeMultiplier={1.4} selectable>{t("isExam")} : {currentItem.isExamine === 0 ? "N" : "Y"}</Text>
